...
import 'package:untitled2/common/repository/base_pagination_repository.dart';
...
class PaginationProvider<
  ...
  Future<void> paginate({
    ...
    // 추가로 데이터 더 가져오기
    // true - 추가로 데이터 더 가져옴
    // false - 새로고침과 같은 효과 (첫번째 데이터), 현재 상태를 덮어씌움
    ...
    try {
      ...
      // 4) CursorPaginationRefetching - 첫번째 페이지 부터 다시 데이터를 가져올 때
      // 5) CursorPaginationFetchMore - 추가 데이터를 paginate 해오라는 요청을 받았을 때

      // 바로 반환 하는 상황
      // 1) hasMore = false (기존 상태에서 이미 다음 데이터가 없다는 값을 알고 있다면)
      // 2) 로딩중 - fetchMore : true
      //    fetchMore가 아닐 때 - 새로고침의 의도가 있을 수 있다.
      if(state is CursorPagination && !forceRefetch) {
        // as는 무조건 이런 타입인 경우에만 써야함 ..!!
